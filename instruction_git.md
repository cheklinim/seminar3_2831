...
## Создание копии стороннего репо

*Для участия в интересующем вас проекте необходимо*:
1. На странице чужого репо нажимаем кнопку "fork" и создаем копию у себя на странице.*
2. *Создаём папку на компьютере для репозитория*
3. *Из этой папки запускаем VSC*
4. *На выбранном репозитории в Github нажимаем кнопку (Code)*
5. *Из открывшегося окна копируем ссылку репозитория*
6. В *VSC набираем команду*:

        git clone <скопированная ссылка>

*произойдёт загрузка и в созданной папке появится репозиторий*
6. *Для перехода к работе с загруженным репозиторием набираем команду*:

        cd <название папки>

*как и при команде add нет необходимости набирать название целиком, достаточно набрать три первых символа и нажать* (Tab)
7. *ОБЯЗАТЕЛЬНО!!! Создаём новую ветку и работаем только в ней!
## Создание пул-реквеста

*Для отправки ваших правок владельцу репо следует:*
1. *Актуализировать версию на Github (запушить). Так как работа проводилась не в ветке "master" при наборе команды "git push" программа предложит сделать это через команду:*

        git push --set-upstream origin <название вашей ветки>
2. *Обновить Github и нажать кнопку "Compare & pull request" или через "Pull request"*
3. *Написать в сообщении к "Pull request" автору о ваших предложениях.* 

    <font size="5">Важно!</font> *Обратите внимание, что*:
*  *Вы работали только в созданной вами ветке. В противном случае, скорее всего, ваши предложения не будут рассмотрены* 
* *Пишите ветку для файла скопированного через fork, такой файл отмечен подписью "Forked from <название репо>*.